...
class TestReservation(TestCase):
    ...
    @patch("builtins.input", side_effect=["y", "Liza", "1", "2", "2", "1", "y"])
    @patch("util.file_reader.FileReader.save_json")
    def test_reserve_dates_successful(self, mock_save_json, mock_input):
        result = reserve_dates(
            self.transient, self.available_dates, self.transients, "test_reservation_list.json"
        )

        self.assertIsNotNone(result)
        mock_save_json.assert_called_once_with(self.transients, "test_reservation_list.json")
